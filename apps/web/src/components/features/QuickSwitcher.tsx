...
import styles from '../../styles/modules/features/QuickSwitcher.module.css'

interface SearchResult {
  id: string
  type: 'channel' | 'server' | 'dm' | 'user' | 'command'
  label: string
  sublabel?: string
...
export function QuickSwitcher({ onClose }: QuickSwitcherProps) {
  ...
  const buildResults = useCallback((q: string): SearchResult[] => {
    ...
    // Commands
    const commands = [
      { id: 'c1', label: '/settings', sublabel: 'Open User Settings', icon: <Settings size={16} />, action: () => { openUserSettingsModal(); onClose() } },
      { id: 'c2', label: '/beacoin', sublabel: 'View Wallet & Store', icon: <Coins size={16} />, action: () => { navigate('/messaging'); window.dispatchEvent(new CustomEvent('open-beacoin-wallet')); onClose() } },
      { id: 'c3', label: '/logout', sublabel: 'Sign out of Beacon', icon: <LogOut size={16} />, action: () => { if (confirm('Logout?')) { /* Logout logic */ }; onClose() } },
      { id: 'c4', label: '/theme', sublabel: 'Switch Visual Theme', icon: <Moon size={16} />, action: () => { /* Cycle theme */ onClose() } }
    ]
