...
export function DMList({ selectedDmId, onSelectDM }: DMListProps) {
  ...
  const getDMInfo = (channel: DMChannel) => {
    ...
    return {
      ...
      avatarDecorationId: (otherUser as any)?.avatarDecorationId,
    }
  }
        ...
        {filteredDMs.length === 0 ? (
          ...
          filteredDMs.map((channel) => {
            ...
            return (
              <button
                key={channel.id}
                className={`${styles.dmItem} ${selectedDmId === channel.id ? styles.active : ''
                  }`}
                onClick={() => handleSelectDM(channel.id)}
              >
                <Avatar src={info.avatar} size="md" status={info.status} avatarDecorationId={info.avatarDecorationId} />
