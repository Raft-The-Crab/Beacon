...
export class BeaconClient extends BeaconEventEmitter {
  ...
  constructor(options: BeaconClientOptions) {
    ...
    // Set default options
    const resolvedApiUrl = resolveApiClientBaseUrl(options.apiUrl)
    const resolvedWsUrl = resolveApiClientGatewayUrl(options.wsUrl, resolvedApiUrl)
    ...
    this.options = {
      ...
      userAgent: options.userAgent || 'BeaconSDK/1.2.0'
    }
